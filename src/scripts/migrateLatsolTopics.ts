...
const run = async () => {
  ...
  for (let i = 0; i < tempQuestions.length; i++) {
    ...
    if (tQuestion.newTopicName === 'Subtype result not found') {
      console.log(
        `Question ${tQuestion.questionId} has no subtype result, moving to UNDECIDED subject`,
      );
      await db
        .update(questions)
        .set({
        ...
        .where(eq(questions.id, tQuestion.questionId))
        .execute();
    } else {
      const question = questionsQuery.find(
        (q) => q.id === tempQuestions[i].questionId,
      );
      console.log(
        `Moving question ${question.id} to new subject: ${tempQuestions[i].newSubjectName} and topic: ${tempQuestions[i].newTopicName}`,
      );
      // update the question subject id and topic id
      await db
        .update(questions)
  ...
  for (let i = 0; i < questionHaveImages.length; i++) {
    console.log(
      `Moving question ${questionHaveImages[i]} that have asset to UNDECIDED subject`,
    );
    await db
      .update(questions)
      .set({
      ...
      .where(eq(questions.id, questionHaveImages[i]))
      .execute();
  }
