...
const run = async () => {
    ...
    for (let i = 0; i < usersQuery.length; i++) {
        ...
        if (dayjs(user.validity_date).isAfter(dayjs(user.onboard_date).add(1, 'day'))) {
            totalPremiumUser += 1;
            ...
            if (userTransaction.length > 0) {
                ...
                userTransaction.sort((a, b) => {
                    return dayjs((a.metadata as any).transaction_time).isAfter(dayjs((b.metadata as any).transaction_time)) ? -1 : 1
                })
                ...
                for (let j = 0; j < userTransaction.length; j++) {
                    ...
                    if ((transaction.metadata as any).transaction_status === 'settlement') {
                ...
                if (latestTransaction) {
                    const orderedSubscriptions = await db.query.transactionOrders.findFirst({
                    ...
                    if (dayjs(user.validity_date).isBefore(dayjs(transactionTime).add(subscriptionsTypeValue[orderedSubscriptions.subscription_type] + 1, 'day'))) {
                        validUsers.push(user)
                    } else {
                        console.log('Latest Transaction: ' + JSON.stringify(latestTransaction.metadata) + '\nValidity Date' + dayjs(user.validity_date).format('YYYY-MM-DD'))

                        invalidUsers.push(user)

                        // adjust the validity date to the latest transaction
                        const newValidityDate = dayjs(transactionTime)
                            .add(subscriptionsTypeValue[orderedSubscriptions.subscription_type], 'day')
                            .toDate();
                    ...
                    console.log('No settled transaction for user ' + user.full_name, user.validity_date)
                    invalidUsers.push(user)
                    // adjust the validity date to the latest transaction
