...
const run = async () => {
  ...
  const q = await db.execute(
    sql`select id, content, topic_id from questions where topic_id = '7ab9324d-6398-4937-8892-e6b3765a51ea' and id not in (select question_id from temporary_topic)`,
  );
  ...
  for (let i = 0; i < 2; i++) {
    ...
    try {
      ...
      const newTopic = await db.execute(
        sql`select id from topics where name = ${data.data.subtype} and subject_id = ${subjectId}`,
      );
