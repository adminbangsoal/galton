...
import { tryout_subjects, tryout_attempts, tryout_sets, tryout_set_attempts, tryouts, tryout_questions } from '../database/schema';
...
const run = async () => {
    ...
    // create tryout
    const tryout = await db.insert(tryouts).values({
    ...
    for (let i = 0; i < subjects.length; i++) {
        const tryoutSet = await db.insert(tryout_sets).values({
            duration: 120,
    ...
    for (let i = 0; i < tryoutSetIds.length; i++) {
        ...
        await db.update(tryout_sets).set({
    ...
    for (let i = 0; i < tryoutSetIds.length; i++) {
        const questions = [];
        const toSet = await db.select().from(tryout_sets).where(eq(tryout_sets.id, tryoutSetIds[i]))
        const setSubject = await db.select().from(tryout_subjects).where(eq(tryout_subjects.id, toSet[0].subjectId))
