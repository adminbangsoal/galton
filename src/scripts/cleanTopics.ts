...
import { questions, subjects, tempQuestionSubject, topics } from "../../src/database/schema";
...
const run = async () => {
    ...
    for (let i = 0; i < tempQuestions.length; i++) {
        ...
        const isTopicIncludesInNewSubject = topicsSubjectMapping[tempQuestion.newSubjectId].includes(tempQuestion.newTopicId);

        if (!isTopicIncludesInNewSubject) {
            const newTopicQ = await db.query.topics.findFirst({
                where: and(eq(topics.name, tempQuestion.newTopicName), eq(topics.subject_id, tempQuestion.newSubjectId)),
                columns: {
                    id: true,
                    name: true
                }
            ...
            if (newTopicQ) {
                ...
                await db.update(tempQuestionSubject).set({
                    newTopicId: newTopicQ.id,
                    newTopicName: newTopicQ.name
                }).where(eq(tempQuestionSubject.id, tempQuestion.id)).execute();
                console.log(`Updated temp question subject id: ${tempQuestion.id} with new topic id: ${newTopicQ.id}`);
            } else {
                console.log(`New topic: ${tempQuestion.newTopicName} is not in the subject: ${tempQuestion.newSubjectName}`);
                // create new topic
                ...
                console.log(`Inserted new topic: ${newTopic[0].name} for subject: ${tempQuestion.newSubjectName}`);
                ...
                console.log(`Updated temp question subject id: ${tempQuestion.id} with new topic id: ${newTopic[0].id}`);
