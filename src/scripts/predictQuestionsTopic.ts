...
import { questions, subjects, topics, tempQuestionSubject } from '../../src/database/schema';
...
const run = async () => {
    ...
    const questionIds = questionAlreadyRunned.map((question) => question.questionId);

    const questionsQuery = await db.query.questions.findMany({
        where: questionIds.length > 0 ? notInArray(questions.id, questionIds) : undefined
    })
    ...
    for (const question of questionsQuery) {
        ...
        const { data } = await axios.post<PredictResponse>('https://ai.bangsoal.co/predict-type-task', {
        ...
        console.log('Successfully predicted topic for question:', question.id, data);

        console.log('=============================================================')

        console.log('INSERTING TO TEMP QUESTION SUBJECT...');

        let newSubject = subjectsQuery.find((subject) => subject.name.toLowerCase() === data.data.type.toLowerCase());

        if (!newSubject) {
            console.log(`Creating new subject: ${data.data.type}`);

            // create the new subject
            const newSubjectQuery = await db.insert(subjects).values({
                name: data.data.type,
        ...
        let newTopic = topicsQuery.find((topic) => topic.name.toLowerCase() === data.data.subtype.toLowerCase() && topic.subject_id === newSubject.id);

        if (!newTopic) {
            console.log(`Creating new topic: ${data.data.subtype} for subject: ${data.data.type}`);

            // create the new topic
            const newTopicQuery = await db.insert(topics).values({
            ...
            console.log(`Inserted new topic: ${newTopic.name} for subject: ${newSubject.name}`);
        }
        ...
        await db.insert(tempQuestionSubject).values({
            ...
            oldSubjectName: subjectsQuery.find((subject) => subject.id === question.subject_id)?.name as string,
            oldTopicId: question.topic_id,
            questionId: question.id,
            oldTopicName: topicsQuery.find((topic) => topic.id === question.topic_id)?.name as string,
