...
const run = async () => {
    ...
    const topics = await db.select().from(t).where(
        eq(t.name, 'Subtype result not found')
    )

    const questions = await db.select().from(q).where(
        inArray(q.topic_id, topics.map(t => t.id))
    )
    ...
    for (let i = 0; i < questions.length; i++) {
        ...
        const { data } = await axios.post<{ data: string }>(`${AI_URL}/predict-type-task`, {
        ...
        const newTopic = await db.select().from(t).where(
            eq(t.name, data.data)
        )

        console.log(`Updating question ${question.id} topic to ${newTopic[0].name}`)
