...
const run = async () => {
  ...
  for (let i = 0; i < rows.length; i++) {
    ...
    const soal = Array.from(
      { length: soalList.length + gambarSoalList.length },
      (_, i) => {
        return {
          content: '',
          isMedia: false,
    ...
    for (let k = 0; k < soal.length; k++) {
      if (k % 2 == 0) {
        // if its even then its not a media
        let latex = '';
        ...
        soal[k].content = getGDriveImageUrl(
          gambarSoalList[gambarSoalListCounter],
        );
        soal[k].isMedia = true;
    ...
    const questionQuery = await db.query.questions.findMany({
      where: and(
        eq(questions.source, insertQuestion.source),
        eq(questions.year, insertQuestion.year),
      ),
    });
    ...
    for (let q = 0; q < questionQuery.length; q++) {
      if (questionQuery[q].question.length === insertQuestion.question.length) {
        ...
        for (let j = 0; j < insertQuestion.question.length; j++) {
          if (
            questionQuery[q].question[j].content !==
            insertQuestion.question[j].content
          ) {
            isSame = false;
            break;
          }
    ...
    const combinedSoal = soal
      .filter((soal) => soal.isMedia === false)
      .map((soal) => soal.content)
      .join('\n');
    ...
    const topicQuery = await db.query.topics.findFirst({
      where: and(
        eq(topics.subject_id, subjectQuery.id),
        eq(topics.name, topic.data.data),
      ),
    });
