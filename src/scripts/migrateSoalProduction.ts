...
const run = async () => {
    ...
    for (let i = 0; i < questionsQuery.length; i++) {
        ...
        if (tQuestion) {
            console.log(`Migrating question ${questionsQuery[i].id} to new topics and subjects...`);
            ...
            if (!subjectQueryMapping[tQuestion.newSubjectName]) {
                ...
                await db.update(questions).set({
                    ...
                    subject_id: newSubject[0].id
                }).where(eq(questions.id, questionsQuery[i].id)).execute();

            } else {
                // check if the topic within the subject is not found, create one
                const topicQuery = await db.query.topics.findFirst({
                    where: and(eq(topics.name, tQuestion.newTopicName), eq(topics.subject_id, subjectQueryMapping[tQuestion.newSubjectName]))
                });

                if (!topicQuery) {
                    console.log(`Creating new topic ${tQuestion.newTopicName}... for subject ${tQuestion.newSubjectName}`);
                    ...
                    const updated = await db.update(questions).set({
                        ...
                        subject_id: subjectQueryMapping[tQuestion.newSubjectName]
                    }).where(eq(questions.id, questionsQuery[i].id)).returning().execute();

                    console.log(`Updated question ${updated[0].id} to new subject: ${tQuestion.newSubjectName} and topic: ${tQuestion.newTopicName}`);
                }
            }
        } else {
            // current question is not in the temp question subject, move it to undecided
            console.log(`Migrating question ${questionsQuery[i].id} to UNDECIDED subject...`);
