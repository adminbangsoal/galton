...
class TryoutWorkerService {
  ...
  async peekEarliestJob() {
    const nextJob = await this.redis.zrange('calculateTryoutScoresJobs', 0, 0, 'WITHSCORES'); // fetch earliest execution time
  ...
  async removeEarliestJob() { // pop operation
    const nextJob = await this.redis.zrange('calculateTryoutScoresJobs', 0, 0, 'WITHSCORES'); // fetch earliest execution time
    if (!nextJob.length) return null;
  ...
  async calculateScoresOfEarliestTryout() {
    ...
    const runningJob = await this.cacheManager.get('calculateTryoutScoresJobsIsRunning');

    if (runningJob) return; // job is already running
    else await this.cacheManager.set('calculateTryoutScoresJobsIsRunning', true); // set job is running

    if (executeJobAt.isBefore(currentTime)) {
      const tryoutId = earliestJob.tryoutId;
      try {
        this.logger.log(`${dayjs().toISOString()}: calculate-tryout-score - Tryout ID: ${tryoutId} - start`);
        await this.calculateUsersTryoutScores(tryoutId);
        this.logger.log(`${dayjs().toISOString()}: calculate-tryout-score - Tryout ID: ${tryoutId} - finished`);
      } catch (err) {
        this.logger.error(`${dayjs().toISOString()}: calculate-tryout-score - Tryout ID: ${tryoutId} - ERROR: ${err.message}`);
  ...
  async calculateUsersTryoutScores(tryoutId: string) {
    ...
    if (!tryoutAttempts.length) {
      this.logger.log(`${dayjs().toISOString()}: calculate-tryout-score - Tryout ID: ${tryoutId} - no user attempts`);
      return;
    }
    ...
    const userSetScores: { [userId: string]: { [setId: string]: number } } = tryoutAttempts.reduce((map, tAttempt) => {
      const defaultSetScores: { [setId: string]: number } = tryoutSets.reduce((map, set) => {
    ...
    if(!questions.length) {
      this.logger.log(`${dayjs().toISOString()}: calculate-tryout-score - Tryout ID: ${tryoutId} - no questions found`);
      return
    }

    // used for mapping the total questions in set, will be used for weight calculation
    const questionsCountSets: { [setId: string]: number } = tryoutSetIds.reduce((map, setId) => {
      map[setId] = 0; // the total questions
      return map;
    }, {});

    // used for mapping the question weights
    const questionsMap: { [questionId: string]: QuestionType } = questions.reduce((map, question) => {
      ...
      map[question.id] = {
        ...
        correctScoreWeight: tryout.isIrt ? question.correctScoreWeight : tryout.correctBasePoint,
        wrongScoreWeight: tryout.isIrt ? question.wrongScoreWeight : tryout.wrongBasePoint,
    ...
    const questionAttempts = await this.db
      ...
      .innerJoin(
        ...
        eq(schema.tryout_set_attempts.userId, schema.tryout_question_attempts.userId)
      )
      .where(and(
        eq(schema.tryout_question_attempts.tryoutId, tryoutId),
        inArray(schema.tryout_question_attempts.userId, tryoutAttempts.map(({ userId }) => userId)),
      ));

    // used for mapping the score for each question attempt
    const qAttemptsMap: { [qAttemptId: string]: QuestionAttemptType } = questionAttempts.reduce((map, attempt) => {
    ...
    if (tryout.isIrt) {
      ...
      Object.keys(questionsMap).forEach(questionId => {
        ...
        const correctPercentage = question.correctAnswersCount / totalTryoutAttempts; // the smaller the value, the question difficulty increases
        ...
        } else if (correctPercentage < 0.50) {
        ...
        } else if (correctPercentage < 0.90) {
    ...
    const userTryoutScores: { [key: string]: number } = Object.keys(userSetScores).reduce((map, userId) => {
      const setScores = userSetScores[userId];
      const totalSetScore = Object.values(setScores).reduce((sum, setScore) => sum + setScore, 0);
      map[userId] = totalSetScore / tryoutSetIds.length; // average set scores
      return map;
    }, {})

    // Bulk update questions (correct and wrong weights)
    this.logger.log(`${dayjs().toISOString()}: calculate-tryout-score - Tryout ID: ${tryoutId} - updating question weights`);
    ...
    questionIds.forEach(questionId => {
      const question = questionsMap[questionId];
      caseWhenCorrectScoreWeight.push(sql`when id = ${questionId} then cast(${question.correctScoreWeight} as double precision)`);
      caseWhenWrongScoreWeight.push(sql`when id = ${questionId} then cast(${question.wrongScoreWeight} as double precision)`);
    })

    caseWhenCorrectScoreWeight.push(sql`end)`);
    caseWhenWrongScoreWeight.push(sql`end)`);

    const correctScoreWeightSql = sql.join(caseWhenCorrectScoreWeight, sql.raw(" "));
    const wrongScoreWeightSql = sql.join(caseWhenWrongScoreWeight, sql.raw(" "));
    ...
    this.logger.log(`${dayjs().toISOString()}: calculate-tryout-score - Tryout ID: ${tryoutId} - updating question attempt scores`);
    ...
    for (let i = 0; i < qAttemptIds.length; i += chunkSize) {
      ...
      await this.db.execute(sql`CREATE TEMP TABLE TEMP_Q_SCORES (id uuid, score double precision);`);
      await this.db.execute(sql`INSERT INTO TEMP_Q_SCORES (id, score) VALUES ${sql.join(insertValue,
        sql`, `)}`);
      await this.db.execute(sql`UPDATE ${schema.tryout_question_attempts} tqa SET score = ts.score FROM TEMP_Q_SCORES ts WHERE tqa.id = ts.id;`);
      await this.db.execute(sql`DROP TABLE TEMP_Q_SCORES;`);
    }

    // Bulk update set attempts (scores)
    this.logger.log(`${dayjs().toISOString()}: calculate-tryout-score - Tryout ID: ${tryoutId} - updating set attempt scores`);
    ...
    for (let i = 0; i < userIds.length; i += chunkSize) {
      ...
      await this.db.execute(sql`CREATE TEMP TABLE TEMP_S_SCORES (tryout_set_id uuid, user_id uuid, score double precision);`);
      await this.db.execute(sql`INSERT INTO TEMP_S_SCORES (tryout_set_id, user_id, score) VALUES ${sql.join(insertValue.flat(), sql`, `)}`);
      await this.db.execute(sql`UPDATE ${schema.tryout_set_attempts} tsa SET score = ts.score FROM TEMP_S_SCORES ts WHERE tsa.tryout_set_id = ts.tryout_set_id AND tsa.user_id = ts.user_id;`);
      await this.db.execute(sql`DROP TABLE TEMP_S_SCORES;`);
    }

    // Bulk update tryout attempts (scores)
    this.logger.log(`${dayjs().toISOString()}: calculate-tryout-score - Tryout ID: ${tryoutId} - updating tryout attempt scores`);
    ...
    for (let i = 0; i < userIds.length; i += chunkSize) {
      ...
      await this.db.execute(sql`CREATE TEMP TABLE TEMP_T_SCORES (user_id uuid, score double precision);`);
      await this.db.execute(sql`INSERT INTO TEMP_T_SCORES (user_id, score) VALUES ${sql.join(insertValue, sql`, `)}`);
      await this.db.execute(sql`UPDATE ${schema.tryout_attempts} ta SET score = ts.score FROM TEMP_T_SCORES ts WHERE ta.user_id = ts.user_id;`);
  ...
  async calculateAllTryoutScores() {
    ...
    for (let i = 0; i < tryouts.length; i++) {
      ...
      try {
        this.logger.log(`${dayjs().toISOString()}: calculate-tryout-score - Tryout ID: ${tryout.id} - start`);
        await this.calculateUsersTryoutScores(tryout.id);
        this.logger.log(`${dayjs().toISOString()}: calculate-tryout-score - Tryout ID: ${tryout.id} - finished`);
      } catch (err) {
        this.logger.error(`${dayjs().toISOString()}: calculate-tryout-score - Tryout ID: ${tryout.id} - ERROR: ${err.message}`);
        this.logger.error(err.stack);
      }
    }
  }
  ...
  isAnswerCorrect(question: QuestionType, attempt: any): boolean {
    ...
    if (question.type == TryoutTypeEnum.MULTIPLE_CHOICE) {
      ...
      isCorrect = question.options.find(option => option.id === selectedOptionId)?.is_true;
    } else if (question.type == TryoutTypeEnum.FILL_IN) {
      isCorrect = JSON.stringify(attempt.filledAnswer) === JSON.stringify(question.correctAnswers);
    } else if (question.type == TryoutTypeEnum.MULTIPLE_ANSWER) {
      const correctedOptionsId = question.options.filter(option => option.is_true).map(option => option.id);
      correctedOptionsId.sort()
      const filledAnswers = attempt.filledAnswer?.map(answer => answer).sort();
      isCorrect = JSON.stringify(correctedOptionsId) === JSON.stringify(filledAnswers);
      ...
      for (let i = 0; i < question.options.length; i++) {
        if (question.options[i].is_true && attempt.filledAnswer[i] != "TRUE") {
          isCorrect = false
          break;
        } else if (!question.options[i].is_true && attempt.filledAnswer[i] != "FALSE") {
          isCorrect = false
          break;
        }
      }
