...
export default class ReferralService {
  ...
  async checkValidReferralCode(code: string, userId: string) {
    ...
    if (dayjs().isAfter(referralCode.expiredAt)) {
      this.logger.error(
        `Referral code ${referralCode.partnerName} is already expired`,
      );
    ...
    if (referralCode.isActive === false) {
      this.logger.error(
        `Referral code ${referralCode.partnerName} is not active`,
      );
    ...
    if (
      ...
      referralUsage.length >= referralCode.maxUsage
    ) {
      this.logger.error(
        `Referral code ${referralCode.partnerName} is already reached max usage`,
      );

      throw new BadRequestException(
        `Kode referral "${code}" sudah mencapai batas penggunaan`,
      );
    }
